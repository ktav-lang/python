...
for k, v in cfg.items():
    if v is None:
        ...
        kind = f"bool={v}"  # bool первым — True это int!
...
одну, пустой отступ внутри скобок выбрасывается — поэтому
форматирование является неподвижной точкой и его безопасно ставить в
pre-commit хук. Порядок ключей не меняется никогда.
...
здесь дополнительной числовой точности: Python различает `int` и
...
Для проверки входа на границах доверия используйте `ktav.loads_strict(s)`:
он применяет правила канонических скаляров спецификации и выбрасывает
`KtavDecodeError` для lossy-записи. Формы canonical writer, например `1e-3`
и `1e10`, принимаются и дают те же native-значения, что и `loads`.
...
конверт ошибки в виде атрибутов: `error`, `reason`, `line`,
`line_text`, `span`, `path`, `body`, `canonical`, `spec_section`.
Отсутствующие сведения — `None`, а не пропущенный атрибут.
...
try:
    ...
    e.error         # "LossyScalar"
    e.body          # "1.10"  — как записано
    e.canonical     # "1.1"   — как будет храниться
    e.spec_section  # "§3.6/§5.2"
```

`span` — это `{"start": …, "end": …}`, смещения в **байтах** по
UTF-8-исходнику, а не в code units UTF-16. `path` — список точных
декодированных сегментов ключа: ключ, буквально названный `a.b`, — это
один сегмент, и он никогда не разрезается. `str(e)` остаётся
человекочитаемым сообщением; сырой конверт вместо него не подставляется.
...
В отличие от биндингов поверх C ABI, здесь нет обхода через JSON:
атрибуты строятся напрямую из полей `ErrorEnvelope` на стороне Rust.
Имена полей совпадают с остальными биндингами, так что при переходе с
Python на Go вы видите ту же структуру.
