...
Синхронизация со спецификацией Ktav 0.7.1 и Rust core 0.7.2.
...
- Каждое исключение, создаваемое биндингом, теперь несёт конверт
  ...
  человекочитаемым.
- `exc.message` — собственное десятое поле конверта (ktav 0.7.2), взятое
  дословно от ядра. `str(exc) == exc.message` по построению; этот
  биндинг всегда использовал собственный рендеринг ядра для `str(exc)`,
  поэтому атрибут здесь избыточен, но сохраняет тот же десятипольный
  вид конверта, что и в остальных языках.
- `ktav.canonical_from_source()` — канонический текст из исходного
  ...
  биндинга для JavaScript, здесь это не даёт выигрыша в точности чисел
  (Python и так различает `int` и `float`), но, как и `emit_canonical`,
  отбрасывает комментарии и пустые строки — в отличие от `format`.
...
- Зависимость Rust теперь использует `ktav = "0.7.2"` (первое
  опубликованное ядро, несущее поле `message` в конверте), а metadata
