...
- Версия пакета переходит на **0.8.0**, синхронно с ядром и
  спецификацией.
- PyPI sdist не включает корпус спецификации, тесты и исходники docs/CI
  для разработки, сохраняя всё необходимое для сборки wheel.
- README примеров генерируется Polydoc на английском, русском и китайском.
