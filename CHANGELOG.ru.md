...
Синхронизация со спецификацией Ktav и Rust core 0.7.1.
...
- `ktav.format()` — форматтер «текст → текст» с сохранением комментариев,
  на основе `format_str` ядра Rust (ktav 0.7.1, issue rust#13). Приводит
  структуру к каноническому виду, сохраняет каждый комментарий дословно,
  схлопывает серии пустых строк до одной и является фиксированной точкой.
- Каждое исключение, создаваемое биндингом, теперь несёт конверт
  ...
  сегментов ключа, никогда не склеенная строка; `str(exc)` остаётся
  человекочитаемым.
...
- Зависимость Rust теперь использует `ktav = "0.7.1"`, а metadata
  спецификации объявляет `0.7.1`; `rust-version` поднят до `1.71`
  (MSRV ktav 0.7).
- Submodule спецификации закреплён на опубликованном коммите Ktav 0.7.0.
- `ktav.__spec_version__` теперь сообщает `0.7.1`.
- Отказы writer'а передают таксономию ядра: NaN/±Infinity сообщает
  ...
  конверт структурированной ошибки раньше не поставлялся).
